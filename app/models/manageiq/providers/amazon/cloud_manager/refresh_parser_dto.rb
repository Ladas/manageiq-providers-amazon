...
class ManageIQ::Providers::Amazon::CloudManager::RefreshParserDto < ManageIQ::Providers::CloudManager::RefreshParserDto
  ...
  def initialize_dto_collections
    ...
    add_dto_collection(ManageIQ::Providers::Amazon::CloudManager::OrchestrationStack,
                       :orchestration_stacks)
    add_dto_collection(ManageIQ::Providers::Amazon::CloudManager::OrchestrationStackOutput,
                       :orchestration_stacks_outputs)
    add_dto_collection(ManageIQ::Providers::Amazon::CloudManager::OrchestrationStackParameter,
                       :orchestration_stacks_parameters)
    add_dto_collection(ManageIQ::Providers::Amazon::CloudManager::OrchestrationStackResource,
                       :orchestration_stacks_resources)
    add_dto_collection(OrchestrationTemplateCfn,
  ...
  def parse_disk(x)
    # TODO remove the need for returning tuple, the id is not used
    return nil, x
  end
  ...
  def parse_instance(instance)
    ...
    new_result = {
      ...
      :flavor              => flavor,
      # :parent_vm           => @data[:miq_templates].lazy_find(instance.image_id),
      :key_pairs           => [@data[:key_pairs].lazy_find(instance.key_name)].compact,
      :location            => "TODO", #@data[:networks].lazy_find("#{uid}__public", :path => [:hostname]),
  ...
  def parse_instance_hardware(instance, flavor)
    ...
    new_result = {
      ...
      :guest_os             => @data[:miq_templates].lazy_find(instance.image_id, :path => [:hardware, :guest_os]),
  ...
  def parse_stack_template(stack)
    ...
    content = OrchestrationTemplate.with_universal_newline(content)
    md5     = OrchestrationTemplate.calc_md5(OrchestrationTemplate.with_universal_newline(content))
